...
For workflows spanning several questionnaires, use the [Form packages API](reference/form-packages-api.md) to create and resume runs. [Embed the package renderer](aidbox-ui-builder-alpha/embedding.md#embedding-a-package) with the existing `<aidbox-form-renderer>` web component using `request-group-id`, or use a generated link in an iframe. The component also supports definition previews through `plan-definition`, progress callbacks, and submission from your application. The [package concepts](form-packages.md) describe ordering, conditions, prefills, and submission.
